...
function setStudentText(v) {
	opener.document.getElementById("pId").value = v
...
<body>
	<div style="align: center; width: 275px; height: 200; margin: auto;">
		<form action="/spamUser/popup" method="post">
				...
				<c:if test="${!empty listSpam}">
					<c:forEach items="${listSpam}" var="spam" varStatus="status">
						<tr>
							...
							<td style="text-align: left;"><c:out value="${spam.id}" /></td>
							...
							<td style="text-align: center;"><input type="button"
								...
								onclick="setStudentText(${spam.id});" /></td>
						</tr>
					</c:forEach>
				</c:if>
				<c:if test="${empty listSpam}">
